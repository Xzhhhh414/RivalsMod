set_attack_value(AT_UTHROW, AG_CATEGORY, 2);
set_attack_value(AT_UTHROW, AG_SPRITE, sprite_get("nspecial2"));
set_attack_value(AT_UTHROW, AG_NUM_WINDOWS, 1);
set_attack_value(AT_UTHROW, AG_HAS_LANDING_LAG, 4);
set_attack_value(AT_UTHROW, AG_OFF_LEDGE, 1);
set_attack_value(AT_UTHROW, AG_AIR_SPRITE, sprite_get("nspecial2"));
set_attack_value(AT_UTHROW, AG_HURTBOX_SPRITE, sprite_get("nspecial_hurt2"));

set_window_value(AT_UTHROW, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_UTHROW, 1, AG_WINDOW_LENGTH, 40);
set_window_value(AT_UTHROW, 1, AG_WINDOW_ANIM_FRAMES, 10);
set_window_value(AT_UTHROW, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_UTHROW, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_weak1"));
set_window_value(AT_UTHROW, 1, AG_WINDOW_SFX_FRAME, 4);


set_num_hitboxes(AT_UTHROW, 1);

set_hitbox_value(AT_UTHROW, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_UTHROW, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UTHROW, 1, HG_WINDOW, 1);
set_hitbox_value(AT_UTHROW, 1, HG_WINDOW_CREATION_FRAME, 6);
set_hitbox_value(AT_UTHROW, 1, HG_LIFETIME, 30);
set_hitbox_value(AT_UTHROW, 1, HG_HITBOX_X, 82);
set_hitbox_value(AT_UTHROW, 1, HG_HITBOX_Y, -38);
set_hitbox_value(AT_UTHROW, 1, HG_WIDTH, 120);
set_hitbox_value(AT_UTHROW, 1, HG_HEIGHT, 120);
set_hitbox_value(AT_UTHROW, 1, HG_PRIORITY, 10);
set_hitbox_value(AT_UTHROW, 1, HG_DAMAGE, 4);
set_hitbox_value(AT_UTHROW, 1, HG_ANGLE, 90);
set_hitbox_value(AT_UTHROW, 1, HG_BASE_KNOCKBACK, 10);
set_hitbox_value(AT_UTHROW, 1, HG_KNOCKBACK_SCALING, .5);
set_hitbox_value(AT_UTHROW, 1, HG_DRIFT_MULTIPLIER, .01);
set_hitbox_value(AT_UTHROW, 1,HG_BASE_HITPAUSE, 2);
set_hitbox_value(AT_UTHROW, 1, HG_VISUAL_EFFECT_Y_OFFSET, -16);
set_hitbox_value(AT_UTHROW, 1, HG_HITSTUN_MULTIPLIER, 1);
set_hitbox_value(AT_UTHROW, 1, HG_EXTRA_HITPAUSE, 4);